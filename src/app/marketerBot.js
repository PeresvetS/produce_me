...
const bot = new Bot(config.marketerBotToken);
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'MARKETER');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'MARKETER');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'MARKETER');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'MARKETER');
