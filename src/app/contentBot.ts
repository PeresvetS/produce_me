...
logger.info('start of content bot');

const bot = new Bot<BotContext>(config.contentBotToken);
bot.use(
  session({
    initial: () => ({
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    const response = await messageService.sendMessage(userId, message, BotType.CONTENT);
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, BotType.CONTENT);
    await dataManagementService.incrementDialogCount(userId);
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const fileUrl = `https://api.telegram.org/file/bot${config.contentBotToken}/${file.file_path}`;
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, BotType.CONTENT);
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, BotType.CONTENT) as FileProcessingResponse;
