...
logger.info(`start of strategy bot`);

const bot = new Bot(config.strategyBotToken);
...
async function startNewDialog(ctx) {
  ...
  if (subscriptionStatus) {
    ctx.reply('Приветствую! Я Воланд, твой AI-ассистент по построению стратегии для экспертов и их брендов. Чем я могу тебе помочь?');
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'STRATEGY');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'STRATEGY');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const fileUrl = `https://api.telegram.org/file/bot${config.strategyBotToken}/${file.file_path}`;
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'STRATEGY');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'STRATEGY');
