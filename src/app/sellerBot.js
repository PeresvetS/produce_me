...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'SELLER');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'SELLER');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'SELLER');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'SELLER');
