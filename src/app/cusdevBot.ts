...
import { BotContext, SessionData, BotType, FileProcessingResponse } from '../types/bot';

const tempDir = process.env.TEMP_DIR || path.join(__dirname, '../../temp');

logger.info('start of cusdev bot');

const bot = new Bot<BotContext>(config.cusdevBotToken);
bot.use(
  session({
    initial: () => ({
...
async function startNewDialog(ctx: BotContext): Promise<void> {
  ...
  if (subscriptionStatus) {
    await ctx.reply('Привет! Я Ир, твой AI-ассистент по customer development. Чем я могу тебе помочь?');
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    const response = await messageService.sendMessage(userId, message, BotType.CUSDEV);
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, BotType.CUSDEV);
    await dataManagementService.incrementDialogCount(userId);
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const fileUrl = `https://api.telegram.org/file/bot${config.cusdevBotToken}/${file.file_path}`;
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, BotType.CUSDEV);
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, BotType.CUSDEV) as FileProcessingResponse;
