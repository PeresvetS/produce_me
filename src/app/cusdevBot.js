...
const bot = new Bot(config.cusdevBotToken);
...
async function startNewDialog(ctx) {
  ...
  if (subscriptionStatus) {
    ctx.reply('Привет! Я Ир, твой AI-ассистент по customer development. Чем я могу тебе помочь?');
    await managementService.incrementNewDialogCount(userId);
    // await subscriptionService.setUserThreadId(userId, 'CUSDEV', null);
  } else {
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'CUSDEV');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'CUSDEV');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'CUSDEV');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'CUSDEV');
