...
logger.info(`start of metho bot`);

const bot = new Bot(config.methoBotToken);
...
async function startNewDialog(ctx) {
  ...
  if (subscriptionStatus) {
    ctx.reply('Привет! Я Злата, твой бот-методолог. Я помогу тебе с упаковкой и докруткой твоего продукта. Расскажи мне о своем проекте и что бы ты хотел улучшить.');
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'METHO');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'METHO');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const fileUrl = `https://api.telegram.org/file/bot${config.methoBotToken}/${file.file_path}`;
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'METHO');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'METHO');
