...
const bot = new Bot<BotContext>(config.producerBotToken);
bot.use(
  session({
    initial: () => ({
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    const response = await messageService.sendMessage(userId, message, BotType.PRODUCER);
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, BotType.PRODUCER);
    await dataManagementService.incrementDialogCount(userId);
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, BotType.PRODUCER);
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, BotType.PRODUCER) as FileProcessingResponse;
