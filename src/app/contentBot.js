...
async function startNewDialog(ctx) {
  ...
  if (subscriptionStatus) {
    ctx.reply('Привет! Я Соня, твой AI-ассистент по созданию контента. Чем я могу тебе помочь?');
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    let response = await messageService.sendMessage(userId, message, 'CONTENT');
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, 'CONTENT');
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, 'CONTENT');
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, 'CONTENT');
