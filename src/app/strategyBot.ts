...
const bot = new Bot<BotContext>(config.strategyBotToken);
bot.use(
  session({
    initial: () => ({
...
bot.on('message:text', async (ctx) => {
  ...
  try {
    ...
    const response = await messageService.sendMessage(userId, message, BotType.STRATEGY);
    logger.info(`Received response from OpenAI for user ${userId}: ${response}`);
    
    await subscriptionService.logMessage(userId, BotType.STRATEGY);
    await dataManagementService.incrementDialogCount(userId);
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    const aiResponse = await messageService.processVoiceMessage(userId, tempFilePath, BotType.STRATEGY);
...
bot.on(['message:document', 'message:photo'], async (ctx) => {
  ...
  try {
    ...
    const { aiResponse, usage } = await fileService.processFile(ctx, userId, BotType.STRATEGY) as FileProcessingResponse;
